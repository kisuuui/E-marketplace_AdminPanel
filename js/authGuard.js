(function () {
    ...
    function updateProfileUI() {
        ...
        const roleRank = window.AppRoles.normalizeRoleRank(profile);
        ...
        document.querySelectorAll("[data-user-role]").forEach(el => {
            ...
            el.setAttribute("data-role-rank", String(roleRank));
            if (el.classList?.contains("role-chip")) {
                el.classList.remove("role-super", "role-admin", "role-seller", "role-user");
                el.classList.add(
                    roleRank === 1 ? "role-super" :
                    roleRank === 2 ? "role-admin" :
                    roleRank === 3 ? "role-seller" :
                    "role-user"
                );
            }
        });
