...
function initVendorData(vendorUid) {
    // Point the listener to the new nested sub-collection
    db.collection('Vendor-product')
      ...
      .onSnapshot(snap => {
        globalProducts = [];
        snap.forEach(d => {
            globalProducts.push({ id: d.id, ...d.data() });
        });
...
window.openAddProductModal = function() {
    ...
    document.getElementById('modalTitle').innerText = "List New Product";
    document.getElementById('saveBtn').innerText = "Submit Listing";
    document.getElementById('saveBtn').onclick = saveNewProduct;
...
window.saveNewProduct = async function() {
    ...
    if (!name || !price) return alert("Please fill in the Name and Price");
    ...
    try {
        ...
        const productData = {
            ...
            // --- CRITICAL VENDOR TAGS ---
            sellerId: auth.currentUser.uid,
            Recipient: currentVendorName || "Staff Member", // Grabs "Library" or "Virtual Lab"
            // ----------------------------
...
// RENDER VENDOR PRODUCTS (Filtered for this seller only)
function renderVendorProducts() {
    const tbody = document.querySelector('#productsTable tbody');
    const fullList = document.getElementById('tbody-all-items');
    if (!tbody) return;
    ...
    globalProducts.forEach(p => {
        const row = `
        <tr class="border-b dark:border-dark-border hover:bg-gray-50 dark:hover:bg-white/5 transition-all">
            <td class="px-8 py-5 flex items-center gap-4">
                <img src="${p.Image}" class="w-10 h-10 rounded-lg object-cover shadow-sm">
                <div>
                    <p class="font-bold text-slate-700 dark:text-white">${p.Product}</p>
                    <p class="text-[10px] text-gray-400 uppercase">${p.Category}</p>
                </div>
            </td>
            <td class="px-8 py-5">
                <span class="font-mono font-bold text-slate-600 dark:text-gray-300">${p.Stock}</span>
            </td>
            <td class="px-8 py-5 text-right">
                <span class="px-2 py-1 rounded text-[10px] font-black uppercase ${p.Status === 'In Stock' ? 'bg-green-100 text-green-600' : 'bg-orange-100 text-orange-600'}">${p.Status}</span>
            </td>
        </tr>`;
...
window.previewImage = function(input) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            placeholder.classList.add('opacity-0'); // Hide placeholder
            if(overlay) overlay.classList.replace('hidden', 'flex'); // Show "Change" overlay
        }
...
// Update your existing clear function to reset the preview too
...
window.saveNewProduct = async function() {
    ...
    // Validation
    if (!name || !price || !stock) return alert("Please fill in all required fields.");
    ...
    try {
        ...
        let imageUrl = "https://ui-avatars.com/api/?name=Product&background=852221&color=fff";
        if (file) {
        ...
        const productData = {
            ...
            Description: desc,
            Image: imageUrl,
            Status: "Pending", // Default for Admin Approval
            sellerId: auth.currentUser.uid,
            vendorName: currentVendorName || "Nexus Vendor", 
            createdAt: firebase.firestore.FieldValue.serverTimestamp()
        };

        // 4. SAVE TO CUSTOM SAVEPOINT (Nested Structure)
        // Path: Vendor-product -> {UID} -> listings -> {Auto-ID}
        await db.collection('Vendor-product')
                .doc(auth.currentUser.uid)
                .collection('listings')
                .add(productData);

        alert("Listing saved successfully and sent for approval!");
        
        // Reset UI
        closeModal('addItemModal');
        resetModalPreview(); // The preview function we made earlier
